import utils.facility_location as craig
import numpy as np
import torch
...
def get_coreset(prediction_error,
                labels,
                N,
                B,
                num_classes,
...
def grad_predictions(loader, model, num_classes, use_linear=False, half_prec=False, return_logits_and_tensors=False):
    ...
    if use_linear:
        ...
        prediction_error = preds - (torch.eye(num_classes))[labels.long()]

    if return_logits_and_tensors:
        return prediction_error, preds, labels, logits
    else:
        return prediction_error.data.numpy(), preds.data.numpy(), labels.data.numpy()
